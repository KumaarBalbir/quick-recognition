...
1. If you have "git" installed, open CMD and type "git clone https://github.com/KumaarBalbir/engage2022.git".
...
4. note down the version of dependencies you have to install. You may face some kind of warning or error if version of dependencies doesn't comply with the used ones.
5. use command : npm install --save <package name>@version --legacy-peer-deps , to install the required version
   of packages.

### Step 3: Create an account in MongoDB Cloud and configure

1. Go to https://www.mongodb.com/try to register a free-tier account.
2. Create a project named "Your_project" and create a cluster named "Your_cluster".
3. Choose the nearest region, for me it was south mumbai.
4. Go to the network access and click on Add IP addrss. Then click on "allow access from anywhere"
...
3. In tab "Media Library", create folder named "quickRecognition".
4. Inside folder "quickRecognition", create another two folders named "ProfilePicture" and "FaceGallery".
5. Click Setting icon on top right.
6. Under tab "Upload", scroll down until you see "Upload presets".
7. Click "Add upload preset", set Upload preset name to "quickrecognition_facegallery" and folder set to "quickRecognition/FaceGallery".
8. Leave the others as default and click "Save".
9. Again, click "Add upload preset", set Upload preset name to "quickrecognition_profilepicture" and folder set to "quickRecognition/ProfilePicture".
...
6. Add the javascript origin: http://localhost:3000, https://quick-recognition.netlify.app/ ..(mainly when you deploy your web app replace the last one link with your own deployed url).
...
13. Add the javascript origin: http://localhost:4000, https://quick-recognition.netlify.app/
...
When you deploy your web app server side on heroku, you have to config all these env variables with
heroku.

1. Using command line(change directory to server side if you are not):
   $ heroku config:set <configVar_key> = <var_value> .

2. using heroku dashboard:
   You can also edit config vars from your app’s Settings tab in the Heroku Dashboard:
   ![Alt text](screenshots/heroku_config.png?raw=true "Heroku config vars")
...
Models purpose:
![Alt text](screenshots/modelpurpose.png?raw=true "Models purpose")

mongoDB collections:
![Alt text](screenshots/db_collection.png?raw=true "db collections")

After you signup, you'll receive welcome mail on your registered email:
![Alt text](screenshots/welcome_mail.png?raw=true "Welcome mail")

Student face registration
![Alt text](screenshots/student_face_registration.png?raw=true "face_registration")

Student profile
![Alt text](screenshots/student_profile.png?raw=true "student profile")

attendance time
![Alt text](screenshots/attendance_time.png?raw=true "attendance_time")

attendance history
![Alt text](screenshots/attendance_transaction.png?raw=true "attendance history")

lecturer add course
![Alt text](screenshots/lecturer_add_course.png?raw=true "add course")

lecturer course attendee
![Alt text](screenshots/lecturer_course.png?raw=true "course attendee")
